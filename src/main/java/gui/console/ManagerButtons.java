package gui.console;

import java.awt.GridLayout;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;

import javax.swing.JLabel;
import javax.swing.JPanel;
import javax.swing.SwingConstants;
import gui.JPanelImageButton;

public class ManagerButtons extends JPanel{
    private String miau="";
    public ManagerButtons() {
        setLayout(new GridLayout(0, 1));

        JPanelImageButton btnOption1 = new JPanelImageButton("shortlist",miau,miau,60,60,0);
        add(btnOption1);

        JPanelImageButton btnOption2 = new JPanelImageButton("generate list",miau,miau,60,60,0);
        add(btnOption2);

        JPanelImageButton btnOption3 = new JPanelImageButton("save details",miau,miau,60,60,0);
        add(btnOption3);

        JPanelImageButton settingsBtn=new JPanelImageButton("settings",miau,miau,60,60,0);
        add(settingsBtn);

        JLabel lblStatus=new JLabel("status");
        lblStatus.setHorizontalAlignment(SwingConstants.CENTER);
        add(lblStatus);

        btnOption1.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                lblStatus.setText("to shortlist");
            }
        });

        btnOption2.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                lblStatus.setText("List generator");
            }
        });

        btnOption3.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                lblStatus.setText("save detail");
            }
        });

        settingsBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                lblStatus.setText("settings");
            }
        });


			/*addComponentListener(new ComponentAdapter() {
				@Override
				public void componentResized(ComponentEvent e) {
					int adjustedFontSizeBody=(int)(getWidth()/17);
					int adjustedFontSizeBody2=(int)(getWidth()/15);
					Font bodyFont=new Font("Comic Sans MS", Font.PLAIN, (int)adjustedFontSizeBody);
					Font bodyFont2=new Font("Consolas", Font.BOLD, (int)adjustedFontSizeBody2);
					btnOption1.setFont(bodyFont);
					btnOption2.setFont(bodyFont);
					btnOption3.setFont(bodyFont);
					settingsBtn.setFont(bodyFont);
					lblStatus.setFont(bodyFont2);
				}
			});*/
    }

}
