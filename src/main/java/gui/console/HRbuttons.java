package gui.console;

import java.awt.Font;
import java.awt.GridLayout;

import javax.swing.JPanel;
import java.awt.event.ComponentAdapter;
import java.awt.event.ComponentEvent;
import javax.swing.JLabel;
import javax.swing.SwingConstants;
import java.awt.event.ActionListener;
import java.awt.event.ActionEvent;
import gui.JPanelImageButton;

public class HRbuttons extends JPanel{
    private String miau="";

    public HRbuttons() {

        setLayout(new GridLayout(0, 1));

        JPanelImageButton btnOption1 = new JPanelImageButton("edit profile",miau,miau,60,60,JPanelImageButton.BOTTOM);
        add(btnOption1);

        JPanelImageButton btnOption2 = new JPanelImageButton("generate list",miau,miau,60,60,JPanelImageButton.BOTTOM);
        add(btnOption2);

        JPanelImageButton btnOption3 = new JPanelImageButton("save details",miau,miau,60,60,JPanelImageButton.BOTTOM);
        add(btnOption3);

        JPanelImageButton settingsBtn=new JPanelImageButton("settings",miau,miau,60,60,JPanelImageButton.BOTTOM);
        add(settingsBtn);

        JLabel lblStatus=new JLabel("status");
        lblStatus.setHorizontalAlignment(SwingConstants.CENTER);
        add(lblStatus);

        btnOption1.addActionListener(arg0 -> lblStatus.setText("Profile edited!"));

        btnOption2.addActionListener(arg0 -> lblStatus.setText("List generated!"));

        btnOption3.addActionListener(arg0 -> lblStatus.setText("Details saved!"));

        addComponentListener(new ComponentAdapter() {
            @Override
            public void componentResized(ComponentEvent e) {
                int adjustedFontSizeBody= getWidth()/17;
                int adjustedFontSizeBody2= getWidth()/15;
                Font bodyFont=new Font("Comic Sans MS", Font.PLAIN, adjustedFontSizeBody);
                Font bodyFont2=new Font("Consolas", Font.BOLD, adjustedFontSizeBody2);
                btnOption1.setFont(bodyFont);
                btnOption2.setFont(bodyFont);
                btnOption3.setFont(bodyFont);
                settingsBtn.setFont(bodyFont);
                lblStatus.setFont(bodyFont2);
            }
        });
    }

}
